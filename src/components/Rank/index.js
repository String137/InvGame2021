...
const RankBase = ({firebase}) => {
    ...
    async function getRank(){
        ...
        if(!(JSON.stringify(userList) === JSON.stringify(userRankList.map(res=>res['username'])))){
            setUserList(userRankList.map(res=>res['username']));
        }
    }
    ...
    return <ol>{userList.map(res=><li>{res}</li>)}</ol>
}
